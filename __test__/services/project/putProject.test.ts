...
describe('putProject', () => {
   ...
   */
  const EndpointUrl = `/api/project/id/${MockProjectRequest.id}`;
  ...
  afterEach(() => {
    expect(apiClient.put).toHaveBeenCalledWith(EndpointUrl, MockProjectRequest);
  });

  it('should return a successful response when the request is correct', async () => {
    vi.mocked(apiClient.put).mockResolvedValueOnce(MockAxiosResponse);
    const response = await putProject({
      idProject: MockProjectRequest.id,
      projectUpdateInput: MockProjectRequest,
    });
